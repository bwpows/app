...
<div>
    ...
    <div class="d-flex justify-space-between flex-wrap">
        <v-card v-for="item in endDateList" :key="item.value" style="min-width: 45%;" class="py-6 body-1 text-center mb-5 rounded-lg" :class="addTask.end_date == item.value?'primary--text base_border':'transparent_border'" @click="addTask.end_date = item.value">
            {{ item.text }}
        </v-card>
    </div>
...
export default {
    ...
    methods: {
        ...
        //     styles.minDate = new Date(), styles.minDate.setFullYear(2010,0,1);
        //     styles.maxDate = new Date(), styles.maxDate.setFullYear(2020,11,31);
